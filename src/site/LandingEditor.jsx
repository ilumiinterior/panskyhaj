...
// Editor dokumentov (PDF). items = [{id,label,file}]
function DocsEditor({ items, onChange }) {
  const update = (i, patch) => onChange(items.map((d, k) => (k === i ? { ...d, ...patch } : d)));
  const remove = (i) => onChange(items.filter((_, k) => k !== i));
  const add = () => onChange([...items, { id: newId('doc'), label: 'Nový dokument', file: '', image: '' }]);
  const move = (i, d) => {
    const j = i + d;
  ...
  return (
    <div className="ilumi-editor__slides">
      {items.map((d, i) => (
        <div className="ilumi-editor__slide" key={d.id}>
          <div className="ilumi-editor__slide-head">
            <span>{d.label || `Dokument ${i + 1}`}</span>
            <span className="ilumi-editor__slide-btns">
              <button type="button" onClick={() => move(i, -1)} disabled={i === 0} title="Hore">↑</button>
              <button type="button" onClick={() => move(i, 1)} disabled={i === items.length - 1} title="Dole">↓</button>
              <button type="button" className="ilumi-editor__del" onClick={() => remove(i)}>✕</button>
            </span>
          </div>
          <input placeholder="Názov (Pôdorys 2.NP)" value={d.label} onChange={(e) => update(i, { label: e.target.value })} />
          <input placeholder="PDF súbor (landing/docs/podorys.pdf)" value={d.file} onChange={(e) => update(i, { file: e.target.value })} />
          <input placeholder="Náhľadový obrázok (voliteľné, landing/docs/podorys.png)" value={d.image} onChange={(e) => update(i, { image: e.target.value })} />
        </div>
      ))}
      <button type="button" onClick={add}>+ Pridať dokument</button>
    </div>
  );
}
  ...
  switch (block.type) {
    ...
    case 'docs':
      return (
        <>
          <label className="ilumi-editor__lbl">Nadpis sekcie (voliteľné)</label>
          <input placeholder="Dokumentácia" value={block.title} onChange={(e) => onUpdate({ title: e.target.value })} />
          <ColorRow label="Farba nadpisu" value={block.titleColor || '#1d2733'} onChange={(v) => onUpdate({ titleColor: v })} />
          <DocsEditor items={block.items} onChange={(items) => onUpdate({ items })} />
          <p className="ilumi-editor__note">PDF súbory ulož do <code>public/landing/docs/</code>.</p>
        </>
      );
    default:
      return null;
...
function makeBlock(type) {
  ...
  switch (type) {
    ...
    case 'docs':
      return { id, type, title: 'Dokumentácia', titleColor: '#1d2733', items: [] };
    default:
      return null;
  }
}
