  ...
  switch (b.type) {
    ...
    case 'docs':
      return {
        ...
        items: (b.items || []).map((d, k) => ({
          id: d.id || newId('doc'),
          label: d.label || `Dokument ${k + 1}`,
          file: d.file || '',
          image: d.image || '', // voliteľný náhľadový obrázok
        })),
      };
    default:
  ...
  switch (b.type) {
    ...
    case 'docs':
      return {
        ...
        ...(b.title ? { title: b.title } : {}),
        ...(b.titleColor ? { titleColor: b.titleColor } : {}),
        items: b.items.map((d) => ({ id: d.id, label: d.label, file: d.file, ...(d.image ? { image: d.image } : {}) })),
      };
    default:
      return null;
  }
