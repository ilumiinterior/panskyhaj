...
export default function Editor({
  ...
  return (
    <div className="ilumi-editor">
          ...
          {tour.nodes.map((n) => (
            <li key={n.id} className={n.id === currentNodeId ? 'is-current' : ''}>
              ...
              <span className="ilumi-editor__nodeactions">
                {floors.length > 1 && (
                  <select
                    className="ilumi-editor__floorsel"
                    value={n.floor || ''}
                    title="Poschodie"
                    onChange={(e) => onSetNodeFloor(n.id, e.target.value)}
                  >
                    {floors.map((f) => (
      ...
      {/* --- Pôdorysy (poschodia) --- */}
      <section className="ilumi-editor__sec">
        <h4>6 · Pôdorysy (poschodia)</h4>
        {floors.map((f) => (
          <div className="ilumi-editor__card" key={f.id}>
            <div className="ilumi-editor__card-head">
              {f.label || 'Poschodie'}
              <button type="button" className="ilumi-editor__del" onClick={() => onRemoveFloor(f.id)}>✕</button>
            </div>
            <input
              placeholder="Názov (napr. Prízemie)"
              value={f.label}
              onChange={(e) => onSetFloorMeta(f.id, { label: e.target.value })}
            />
            <input
              placeholder="Obrázok pôdorysu (podorys-prizemie.png)"
              value={f.image}
              onChange={(e) => onSetFloorMeta(f.id, { image: e.target.value })}
            />
          </div>
        ))}
        {floors.length === 0 && (
          <p className="ilumi-editor__note">Žiadny pôdorys — pridaj poschodie nižšie.</p>
        )}
        <button type="button" onClick={onAddFloor}>+ Pridať poschodie</button>
        <p className="ilumi-editor__note">
          Poschodie priradíš pohľadu v sekcii „1 · Miestnosti" (rozbaľovací zoznam).
          Potom klikni do pôdorysu vpravo dole — umiestni sem aktuálnu miestnosť na jej poschodie.
        </p>
      </section>
