...
export default function Tour({ src, edit = false }) {
  ...
  const addNode = useCallback((name, file) => {
    ...
    setTour((prev) => {
      ...
      // predvolené poschodie = to, na ktorom stojí aktuálny pohľad (alebo prvé)
      const curFloor = prev.nodes.find((n) => n.id === currentNodeId)?.floor;
      const floor = curFloor || prev.floors[0]?.id || null;
      const node = { id, name, panorama: file, floor, links: [] };
  ...
  // Umiestni aktuálnu miestnosť na pôdorys — na poschodie, na ktorom pohľad stojí.
  const setRoom = useCallback((nodeId, x, y) => {
    if (!nodeId) return;
    setTour((prev) => {
      const node = prev.nodes.find((n) => n.id === nodeId);
      const floorId = node?.floor || prev.floors[0]?.id;
      if (!floorId) return prev;
      const floors = prev.floors.map((f) => {
        if (f.id !== floorId) return f;
        const rooms = f.rooms.filter((r) => r.nodeId !== nodeId).concat({ nodeId, x, y, label: node?.name });
        return { ...f, rooms };
      });
      return { ...prev, floors };
    });
  ...
  // Priradí pohľadu poschodie (a zmaže jeho starú pozíciu z ostatných poschodí).
  const setNodeFloor = useCallback((nodeId, floorId) => {
    setTour((prev) => ({
      ...prev,
      nodes: prev.nodes.map((n) => (n.id === nodeId ? { ...n, floor: floorId } : n)),
      floors: prev.floors.map((f) =>
        f.id === floorId ? f : { ...f, rooms: f.rooms.filter((r) => r.nodeId !== nodeId) },
      ),
    }));
  }, []);

  // Správa poschodí (pôdorysov).
  const addFloor = useCallback(() => {
    setTour((prev) => {
      const floors = prev.floors || [];
      return {
        ...prev,
        floors: [...floors, { id: `floor-${Date.now()}`, label: `Poschodie ${floors.length + 1}`, image: '', rooms: [] }],
      };
    });
  }, []);

  const setFloorMeta = useCallback((floorId, patch) => {
    setTour((prev) => ({
      ...prev,
  ...
  const removeFloor = useCallback((floorId) => {
    setTour((prev) => {
      ...
      const fallback = floors[0]?.id || null;
      const nodes = prev.nodes.map((n) => (n.floor === floorId ? { ...n, floor: fallback } : n));
      return { ...prev, floors, nodes };
    });
  }, []);
  ...
  const removeNode = useCallback((nodeId) => {
    ...
    // zmaž bod tejto miestnosti zo všetkých poschodí
    const floors = (prev.floors || []).map((f) => ({ ...f, rooms: f.rooms.filter((r) => r.nodeId !== nodeId) }));
  ...
  // aktívny pôdorys = poschodie, na ktorom stojí aktuálny pohľad (alebo prvé)
  const activeFloor =
    tour?.floors?.find((f) => f.id === currentNode?.floor) || tour?.floors?.[0] || null;
    ...
    <div className={`ilumi-tour ${edit ? 'is-edit' : ''}`}>
      ...
      {activeFloor?.image && (
        <FloorPlan
          ...
          multiFloor={tour.floors.length > 1}
          baseUrl={baseUrl}
      ...
      {edit && tour && (
        <Editor
          ...
          onSetNodeFloor={setNodeFloor}
          onAddFloor={addFloor}
          onSetFloorMeta={setFloorMeta}
