...
/** Znormalizuje miestnosť (bod) na pôdoryse. */
...
export function normalizeTour(raw) {
  // Pôdorysy (poschodia). Nové: raw.floors[]. Staré: raw.floorplan (1 pôdorys)
  // sa prevedie na jedno poschodie.
  let floors = [];
  if (Array.isArray(raw.floors) && raw.floors.length) {
    floors = raw.floors.map((f, i) => ({
      id: f.id || `floor-${i + 1}`,
      label: f.label || `Poschodie ${i + 1}`,
      image: f.image || '',
      rooms: (f.rooms || []).map(normalizeRoom),
    }));
  } else if (raw.floorplan) {
    floors = [
      {
        id: 'floor-1',
        label: raw.floorplan.label || 'Pôdorys',
        image: raw.floorplan.image || '',
        rooms: (raw.floorplan.rooms || []).map(normalizeRoom),
      },
    ];
  }
  const defaultFloor = floors[0]?.id || null;
  ...
  const nodes = (raw.nodes || []).map((n) => ({
    ...
    floor: n.floor || defaultFloor, // poschodie, na ktorom pohľad stojí
    links: (n.links || []).map((l) => ({
      nodeId: l.nodeId,
      yaw: Number(l.yaw) || 0,
